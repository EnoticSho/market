angular.module('app', ['ngStorage']).controller('indexController', function ($scope, $http, $localStorage) {
    ...
    $scope.addToCart = function (productId) {
        $http.post('http://localhost:8081/cart/api/v1/cart/' + productId).then(function () {
            $scope.loadProductsInCart();
        });
    }

    $scope.loadProductsInCart = function () {
        $http.get('http://localhost:8081/cart/api/v1/cart/').then(function (response) {
            $scope.cart = response.data;
        });
    }

    $scope.deleteProductFromCart = function (productId) {
        $http.delete('http://localhost:8081/cart/api/v1/cart/' + productId).then(function () {
            $scope.loadProductsInCart();
        });
    }

    $scope.clearTheCart = function () {
        $http.delete('http://localhost:8081/cart/api/v1/cart/').then(function () {
            $scope.loadProductsInCart();
        });
    }
    ...
    $scope.incrementQuantity = function (productId, delta) {
        $http({
            url: 'http://localhost:8081/cart/api/v1/cart/increment',
            method: 'Put',
            params: {
                productId: productId,
                delta: delta
