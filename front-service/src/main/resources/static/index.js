angular.module('app', ['ngStorage']).controller('indexController', function ($scope, $http, $localStorage) {
    ...
    $scope.checkAuth = function () {
        $http.get('http://localhost:5555/auth_check').then(function (response) {
    ...
    $scope.deleteProductById = function (productId) {
        $http.delete('http://localhost:5555/core/api/v1/products/' + productId).then(function () {
            $scope.loadProducts();
        });
    }
    ...
    $scope.deleteProductFromCart = function (productId) {
        $http.get('http://localhost:5555/cart/api/v1/cart/delete/' + productId).then(function () {
            $scope.loadProductsInCart();
        });
    }

    $scope.clearTheCart = function () {
        $http.get('http://localhost:5555/cart/api/v1/cart/deleteCart').then(function () {
            $scope.loadProductsInCart();
        });
    }
    ...
    $scope.createOrder = function () {
        $http({
            url: 'http://localhost:5555/core/api/v1/purchase',
            method: 'POST',
        }).then(function () {
            $scope.clearTheCart();
            $scope.loadProductsInCart();
